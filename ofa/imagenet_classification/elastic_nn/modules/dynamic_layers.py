...
from ofa.utils.layers import ResNetBottleneckBlock, LinearLayer, InceptionBlock
...
class DynamicMBConvLayer(MyModule):
    ...
    def config(self):
        return {
            ...
            'use_se': self.use_se,
            'active': self.get_active_subnet_config(self.active_in_channel)
        }
...
class DynamicConvLayer(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list, kernel_size_list, stride=1, dilation=1,
        ...
        self.conv = DynamicConv2d(
            ...
            kernel_size_list=self.kernel_size_list, stride=self.stride, dilation=self.dilation,
        )
        if self.use_bn:
    ...
    @property
    def module_str(self):
        return 'DyConv(O%d, K%d, S%d)' % (self.active_out_channel, self.active_kernel_size, self.stride)
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        sub_layer.conv.weight.data.copy_(self.conv.get_active_filter(self.active_out_channel, in_channel, self.active_kernel_size).data)
        if self.use_bn:
...
class DynamicResNetBottleneckBlock(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list, expand_ratio_list=0.25,
                 kernel_size_list=[3], stride=1, act_func='relu', downsample_mode='avgpool_conv'):
        ...
        self.conv2 = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d(max_middle_channel, max_middle_channel, kernel_size_list, stride)),
            ('bn', DynamicBatchNorm2d(max_middle_channel)),
            ('act', build_activation(self.act_func, inplace=True))
        ]))
    ...
    def forward(self, x):
        ...
        self.conv2.conv.active_kernel_size = self.active_kernel_size
    ...
    def module_str(self):
        return '(%s, %s)' % (
            '%dx%d_BottleneckConv_in->%d->%d_S%d' % (
                self.active_kernel_size, self.active_kernel_size, self.active_middle_channels, self.active_out_channel, self.stride
            ),
    ...
    def config(self):
        return {
            ...
            'active': self.get_active_subnet_config(self.in_channels),
        }
    ...
    def active_middle_channels(self):
        #print("active_expand_ratio {}".format(self.active_expand_ratio))
        #print("active_out_channel {}".format(self.active_out_channel))
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        sub_layer.conv2.conv.weight.data.copy_(
            self.conv2.conv.get_active_filter(
                self.active_middle_channels, self.active_middle_channels, self.active_kernel_size
            ).data
        )
        ...
        sub_layer.conv2.conv.weight.data.copy_(
            self.conv2.conv.get_active_filter(self.active_middle_channels, self.active_middle_channels, self.active_kernel_size).data)
        copy_bn(sub_layer.conv2.bn, self.conv2.bn.bn)
...
class DynamicInceptionBlock(MyModule):

    def __init__(self, in_channel_list, out_channel_list, out_channel_sub_list,
                 stride=1, act_func='relu', ):
        super(DynamicInceptionBlock, self).__init__()

        self.in_channel_list = in_channel_list
        self.out_channel_list = out_channel_list
        self.out_channel_sub_list = out_channel_sub_list


        self.stride = stride
        self.act_func = act_func

        self.ch1x1_out_channel_list = []
        self.ch3x3red_out_channel_list = []
        self.ch3x3_out_channel_list = []
        self.ch5x5red_out_channel_list = []
        self.ch5x5_out_channel_list = []
        self.pool_proj_out_channel_list = []

        for x in out_channel_sub_list:
            self.ch1x1_out_channel_list.append(x[0])
            self.ch3x3red_out_channel_list.append(x[1])
            self.ch3x3_out_channel_list.append(x[2])
            self.ch5x5red_out_channel_list.append(x[3])
            self.ch5x5_out_channel_list.append(x[4])
            self.pool_proj_out_channel_list.append(x[5])
        '''
        # build modules
        max_middle_channel = make_divisible(
            round(max(self.out_channel_list) * max(self.expand_ratio_list)), MyNetwork.CHANNEL_DIVISIBLE)
        ...
        self.branch1 = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d(max(self.in_channel_list), max(self.ch1x1_out_channel_list), kernel_size_list = [1], stride = stride)),
            ('bn', DynamicBatchNorm2d(max(self.ch1x1_out_channel_list))),
            ('act', build_activation(self.act_func, inplace=True)),
        ]))

        self.branch2 = nn.Sequential(OrderedDict([
            ('conv1', DynamicConv2d(max(self.in_channel_list), max(self.ch3x3red_out_channel_list), kernel_size_list = [1], stride = stride)),
            ('conv2', DynamicConv2d(max(self.ch3x3red_out_channel_list), max(self.ch3x3_out_channel_list), kernel_size_list=[3],
                                   stride=stride)),
            ('bn', DynamicBatchNorm2d(max(self.ch3x3_out_channel_list))),
            ('act', build_activation(self.act_func, inplace=True))
        ]))

        self.branch3 = nn.Sequential(OrderedDict([
            ('conv1', DynamicConv2d(max(self.in_channel_list), max(self.ch5x5red_out_channel_list), kernel_size_list=[1],
                                   stride=stride)),
            ('conv2', DynamicConv2d(max(self.ch5x5red_out_channel_list), max(self.ch5x5_out_channel_list), kernel_size_list=[3],
                                   stride=stride)),
            ('bn', DynamicBatchNorm2d(max(self.ch5x5_out_channel_list))),
            ('act', build_activation(self.act_func, inplace=True))
        ]))

        self.branch4 = nn.Sequential(OrderedDict([
            ('maxpool', nn.MaxPool2d(kernel_size=3, stride=1, padding=1, ceil_mode=True)),
            ('conv', DynamicConv2d(max(self.in_channel_list), max(self.pool_proj_out_channel_list), kernel_size_list=[1],
                                   stride=stride)),
            ('bn', DynamicBatchNorm2d(max(self.pool_proj_out_channel_list))),
            ('act', build_activation(self.act_func, inplace=True))
        ]))

        self.final_act = build_activation(self.act_func, inplace=True)
        self.active_out_channel = max(self.out_channel_list)
        self.active_ch1x1_out_channel = max(self.ch1x1_out_channel_list )
        self.active_ch3x3red_out_channel = max(self.ch3x3red_out_channel_list )
        self.active_ch3x3_out_channel = max(self.ch3x3_out_channel_list )
        self.active_ch5x5red_out_channel = max(self.ch5x5red_out_channel_list )
        self.active_ch5x5_out_channel = max(self.ch5x5_out_channel_list )
        self.active_pool_proj_out_channel = max(self.pool_proj_out_channel_list )

    def forward(self, x):

        self.branch1.conv.active_out_channel = self.active_ch1x1_out_channel
        self.branch2.conv1.active_out_channel = self.active_ch3x3red_out_channel
        self.branch2.conv2.active_out_channel = self.active_ch3x3_out_channel
        self.branch3.conv1.active_out_channel = self.active_ch5x5red_out_channel
        self.branch3.conv2.active_out_channel = self.active_ch5x5_out_channel
        self.branch4.conv.active_out_channel = self.active_pool_proj_out_channel

        #print('branch input', x.size())
        #print(self.branch2)
        #print(self.branch3)
        #print(self.branch3.conv1.active_out_channel)
        #print(self.branch3.conv2.active_out_channel)
        x1 = self.branch1(x)
        x2 = self.branch2(x)
        #print('branch input', x.size())
        x3 = self.branch3(x)
    ...
    def module_str(self):
        return '(%s)' % (
            'InceptionBlock_%d->%d->%d_%d_%d_%d_%d_%d_%d' % (
                self.in_channels, self.active_out_channel, self.active_ch1x1_out_channel, self.active_ch3x3red_out_channel, self.active_ch3x3_out_channel,
                self.active_ch5x5red_out_channel, self.active_ch5x5_out_channel,
                self.active_pool_proj_out_channel, self.stride
            ),
        )
    ...
    def config(self):
        return {
            'name': DynamicInceptionBlock.__name__,
            'in_channel_list': self.in_channel_list,
            'out_channel_list': self.out_channel_list,
            'out_channel_sub_list': self.out_channel_sub_list,
            'stride': self.stride,
            'active': self.get_active_subnet_config(self.in_channels),  # Check and rewrite
        }

    @staticmethod
    def build_from_config(config):
        return DynamicInceptionBlock(**config)  # Check and rewrite
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        # copy weight from current layer
        sub_layer.branch1.conv.weight.data.copy_(
            self.branch1.conv.get_active_filter(
                self.active_ch1x1_out_channel, in_channel
            ).data
        )
        copy_bn(sub_layer.branch1.bn, self.branch1.bn.bn)

        sub_layer.branch2.conv1.weight.data.copy_(
            self.branch2.conv1.get_active_filter(
                self.active_ch3x3red_out_channel, in_channel
            ).data
        )
        sub_layer.branch2.conv2.weight.data.copy_(
            self.branch2.conv2.get_active_filter(
                self.active_ch3x3_out_channel, self.active_ch3x3red_out_channel
            ).data
        )
        copy_bn(sub_layer.branch2.bn, self.branch2.bn.bn)

        sub_layer.branch3.conv1.weight.data.copy_(
            self.branch3.conv1.get_active_filter(
                self.active_ch5x5red_out_channel, in_channel
            ).data
        )
        sub_layer.branch3.conv2.weight.data.copy_(
            self.branch3.conv2.get_active_filter(
                self.active_ch5x5_out_channel, self.active_ch5x5red_out_channel
            ).data
        )
        copy_bn(sub_layer.branch3.bn, self.branch3.bn.bn)

        sub_layer.branch4.conv1.weight.data.copy_(
            self.branch4.conv1.get_active_filter(
                self.active_pool_proj_out_channel, in_channel
            ).data
        )

        copy_bn(sub_layer.branch4.bn, self.branch4.bn.bn)
    ...
    def get_active_subnet_config(self, in_channel):
        return {
            'name': InceptionBlock.__name__,
            'in_channels': self.in_channels,
			'out_channels': self.active_out_channel,
            'out_channel_sub_list' : [self.active_ch1x1_out_channel,self.active_ch3x3red_out_channel,
                                      self.active_ch3x3_out_channel,self.active_ch5x5red_out_channel,
                                      self.active_ch5x5_out_channel,self.active_pool_proj_out_channel],
			'stride': self.stride,
    ...
    '''
                'channels_1x1': self.active_ch1x1_out_channel,
    			'channels_3x3reduce': self.active_ch3x3red_out_channel,
    			'channels_3x3': self.active_ch3x3_out_channel,
    			'channels_5x5red': self.active_ch5x5red_out_channel,
    			'channels_5x5': self.active_ch5x5_out_channel,
    			'pool_projection': self.active_pool_proj_out_channel,
    '''

    def re_organize_middle_weights(self,expand_ratio_stage=0): # Write this function
        # branch1  conv rearrange
        importance = torch.sum(torch.abs(self.branch1.conv.conv.weight.data), dim=(0, 2, 3))

        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        self.branch1.conv.conv.weight.data = torch.index_select(self.branch1.conv.conv.weight.data, 1, sorted_idx)

        # branch 2 conv2 -> conv1
        importance = torch.sum(torch.abs(self.branch2.conv2.conv.weight.data), dim=(0, 2, 3))

        if expand_ratio_stage > 0:
            sorted_expand_list = copy.deepcopy(self.expand_ratio_list)
            sorted_expand_list.sort(reverse=True)
            target_width_list = [
                make_divisible(round(max(self.ch3x3_out_channel_list) * expand), MyNetwork.CHANNEL_DIVISIBLE)
                for expand in sorted_expand_list
            ]
            right = len(importance)
            base = - len(target_width_list) * 1e5
        ...
        self.branch2.conv2.conv.weight.data = torch.index_select(self.branch2.conv2.conv.weight.data, 1, sorted_idx)
        #adjust_bn_according_to_idx(self.branch2.bn.bn, sorted_idx)
        self.branch2.conv1.conv.weight.data = torch.index_select(self.branch2.conv1.conv.weight.data, 0, sorted_idx)

        # branch 3 conv2 -> conv1
        importance = torch.sum(torch.abs(self.branch3.conv2.conv.weight.data), dim=(0, 2, 3))

        if expand_ratio_stage > 0:
            sorted_expand_list = copy.deepcopy(self.expand_ratio_list)
            sorted_expand_list.sort(reverse=True)
            target_width_list = [
                make_divisible(round(max(self.ch5x5_out_channel_list) * expand), MyNetwork.CHANNEL_DIVISIBLE)
                for expand in sorted_expand_list
            ]
            right = len(importance)
            base = - len(target_width_list) * 1e5
        ...
        self.branch3.conv2.conv.weight.data = torch.index_select(self.branch3.conv2.conv.weight.data, 1,
                                                                      sorted_idx)
        #adjust_bn_according_to_idx(self.branch3.bn.bn, sorted_idx)
        self.branch3.conv1.conv.weight.data = torch.index_select(self.branch3.conv1.conv.weight.data, 0,
                                                                      sorted_idx)

        # branch4 conv rearrange
        importance = torch.sum(torch.abs(self.branch4.conv.conv.weight.data), dim=(0, 2, 3))

        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        self.branch4.conv.conv.weight.data = torch.index_select(self.branch4.conv.conv.weight.data, 1,
                                                                     sorted_idx)
